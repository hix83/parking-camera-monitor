...
- перед публичным релизом Telegram-токен лучше перевыпустить
- на сервере стоит явно настроить `date.timezone` в `php.ini`
- если камера или сеть нестабильны, держите `capture_timeout_seconds` в разумных пределах
